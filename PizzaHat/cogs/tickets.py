...
class TicketView(ui.View):
    ...
    async def get_staff_role(self, guild_id: int):
        await self.bot.db.fetchval("SELECT role FROM staff_role WHERE guild_id=$1", guild_id)
    
    @ui.button(emoji="<:ticket_emoji:1004648922158989404>", custom_id="create_ticket_btn")
    async def create_ticket(self, interaction: Interaction, button: ui.Button):
        ...
        if interaction.guild is not None:
            ...
            await interaction.response.edit_message(content=f"Ticket created! {channel.mention}")
...
class Tickets(Cog, emoji="🎟"):
    ...
    async def ticket(self, ctx: Context):
        ...
        await ctx.send("Please use the command `p!setup tickets <#channel>` to setup ticket system in this server.")
