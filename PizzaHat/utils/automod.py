...
class AutoMod(Cog):
    ...
    async def check_if_am_is_enabled(self, guild_id):
        data = await self.bot.db.fetchval("SELECT enabled FROM automod WHERE guild_id=$1", guild_id)
        if data:
            return data
    ...
    async def on_automod_trigger(self, msg: discord.Message, module: str):
        logs_channel = self.bot.get_channel(self.get_logs_channel(msg.guild.id))
